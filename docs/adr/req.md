...
스트리머카드에서 포지션과 전적 삭제. 계정 레벌 더 명확하게
사걱형 스트리머 카드를 티어리스트의 육각형 스트리머 항목과 같은 컴포넌트를 쓰도록
육각형은 치지직 api 연동으로 프로필 사진이 들어가돼 없으면 지금처럼 닉네임이 들어가도록
스트리머 페이지에의 스트리머 카드 하단에 그라데이션을 넣고 그 위에 스트리머 닉네임과 계정레벨을 기입

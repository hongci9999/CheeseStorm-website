...
import {
  ...
  heroesPickedInSeries, heroesPickedByTeam, buildDefaultAssignment, swapAssignment,
...
export function DraftBoard({ series, state, onApply, onUndo, onFinish }: Props) {
  ...
  // 하드 피어리스: 이전 세트에서 이미 소비된(전역 잠긴) 영웅. 표시용.
  const hardUsed = useMemo(
    () => series.draftType === 'hard' ? [...heroesPickedInSeries(series.sets)] : [],
    [series.draftType, series.sets],
  );

  // 소프트 피어리스: 팀별로 이전 세트에서 픽해 그 팀만 잠긴 영웅. 표시용.
  const softLocked = useMemo(
    () => series.draftType === 'soft'
      ? { blue: [...heroesPickedByTeam(series.sets, 'blue')], red: [...heroesPickedByTeam(series.sets, 'red')] }
      : { blue: [], red: [] },
    [series.draftType, series.sets],
  );
  ...
  // 픽 교환 — 같은 팀 두 슬롯 클릭으로 영웅 스왑. 소프트는 팀 단위 잠금이라 교환 제약 없음.
  function handleSlotClick(team: Team, i: number) {
    ...
    const next = swapAssignment(state, team, selectedSlot.i, i);
  ...
  return (
    <div style={{ maxWidth: 1120, margin: '0 auto', display: 'grid', gap: 'var(--sp-5)', justifyItems: 'center', paddingBottom: 88 }}>
      {/* ── 상단바 아래: 양팀 밴 스트립 (가운데=하드 전역잠금 / 각 팀=소프트 팀잠금) ── */}
      <BanStrip series={series} state={state} hardUsed={hardUsed} softLocked={softLocked} />
...
// 양팀 밴 스트립 — 상단바 아래. 좌 블루 밴 / 가운데 피어리스 잠금 / 우 레드 밴.
// 가운데 = 하드는 전역 잠금 한 묶음, 소프트는 팀별 잠금을 블루·레드 그룹으로 나눠 표시.
function BanStrip({ series, state, hardUsed, softLocked }: {
  series: Series; state: DraftState; hardUsed: string[]; softLocked: { blue: string[]; red: string[] };
}) {
  const hasSoft = softLocked.blue.length > 0 || softLocked.red.length > 0;
  return (
    <div style={{ width: '100%', maxWidth: 1000, display: 'grid', gridTemplateColumns: 'auto 1fr auto',
      ...
      {hardUsed.length > 0 ? (
        ...
        <FearlessBox>
          {softLocked.blue.length > 0 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap' }}>
              {softLocked.blue.map((h) => <HeroHex key={`b-${h}`} hero={h} size={28} tone="soft" ring={teamColor('blue')} />)}
            </div>
          )}
          {softLocked.blue.length > 0 && softLocked.red.length > 0 && (
            <span aria-hidden style={{ width: 1, alignSelf: 'stretch', minHeight: 24, background: 'var(--border-line)' }} />
          )}
          {softLocked.red.length > 0 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap' }}>
              {softLocked.red.map((h) => <HeroHex key={`r-${h}`} hero={h} size={28} tone="soft" ring={teamColor('red')} />)}
            </div>
          )}
        </FearlessBox>
      ) : <span />}
...
// 가운데 피어리스 잠금 묶음 — 점선 박스 + "피어리스" 라벨.
function FearlessBox({ children }: { children: ReactNode }) {
  return (
    <div style={{ justifySelf: 'center', display: 'flex', alignItems: 'center', gap: 'var(--sp-3)', flexWrap: 'wrap', justifyContent: 'center',
      ...
      <span style={{ fontFamily: 'var(--font-ui)', fontSize: 'var(--fs-2xs)', color: 'var(--text-faint)',
        letterSpacing: 'var(--ls-caps)', textTransform: 'uppercase' }}>피어리스</span>
      {children}
    </div>
  );
}
...
// 팀 슬롯 패널 — 지그재그 5칸. 스트리머 위치는 로스터 순서로 고정(픽 전엔 흐림).
  ...
  return (
    <div style={{ width: S + oddOffset, display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
      {Array.from({ length: 5 }).map((_, i) => {
        ...
        // 다른 슬롯이 선택된 상태에서 같은 팀 슬롯은 교환 후보(강조).
        const candidate = selectable && !!selectedSlot && !isSelected && selectedSlot.team === team;
...
// 영웅 육각 칩 — 밴/기사용/소프트잠금 표시. 톤별 테두리색(ring으로 팀색 등 오버라이드 가능).
function HeroHex({ hero, size, tone, ring }: { hero: string; size: number; tone: 'ban' | 'used' | 'soft'; ring?: string }) {
  const toneRing = tone === 'ban' ? 'var(--loss)' : tone === 'soft' ? 'var(--hots-purple)' : 'var(--text-faint)';
  return <span title={hero} style={{ display: 'inline-flex' }}>
    <HexAvatar name={hero} imageUrl={heroImageUrl(hero)} ring={ring ?? toneRing} size={size} />
  </span>;
}
